                        ...
                        if 'img' in html_item or 'video' in html_item:
                            ...
                            if text_title2:
                                ...
                                if len(text_title2) < 30:
                                    ret = pre(text_title2, 25, html_item, repeatid, '',max_count) #内容上传
                                    max_count += 1
                            elif text_title:
                                if '<br/>' in text_title:
                                    text_title = text_title.replace('<br/>', '')
                                if len(text_title) < 30:
                                    ret = pre(text_title, 25, html_item, repeatid, '',max_count) #内容上传
                                    max_count += 1
                            print('max_count',max_count)
                            if max_count == 100:
                                print('break')
                                sys.exit(0)
                    save_weibo(url, 25,i,mid[0])
                    time.sleep(5)
